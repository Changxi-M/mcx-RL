...
from humanoid.tester.testers.tester_base import TesterTypeBase

# 简化的模块导入函数
def get_module_path_from_files_in_dir(root_dir, current_dir):
    """简化的模块路径获取函数"""
    import_modules = {}
    if os.path.exists(current_dir):
        for file in os.listdir(current_dir):
            if file.endswith('.py') and not file.startswith('__'):
                module_name = file[:-3]
                module_path = f"humanoid.tester.testers.{module_name}"
...
from humanoid.wrapper.record_video_wrapper import RecordVideoWrapper
...
import_modules = get_module_path_from_files_in_dir(
    LEGGED_GYM_ROOT_DIR, current_file_directory
)
imported_classes = {}
